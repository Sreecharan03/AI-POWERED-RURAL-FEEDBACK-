    ...
    async def log_conversation_step(self, conversation_data: Dict[str, Any]) -> bool:
        ...
        try:
            payload = {
                ...
                'question_count': conversation_data.get('question_count'),
                'ts': conversation_data.get('timestamp', datetime.now(timezone.utc).isoformat()),
                'audio_metadata': conversation_data.get('audio_metadata'),
                'ai_reasoning': conversation_data.get('ai_reasoning'),
                'ai_fallback_used': conversation_data.get('ai_fallback_used'),
                'ai_confidence': conversation_data.get('ai_confidence'),
            }
            self.db.supabase_client.table('conversation_logs').insert(payload).execute()
            return True
        except Exception as e:
