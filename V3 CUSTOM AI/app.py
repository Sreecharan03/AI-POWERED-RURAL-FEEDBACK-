...
class EnhancedJanSpandanaApp:
    ...
    def check_backend_health(self) -> dict:
        ...
        try:
            health_response = requests.get(f"{self.api_base}/health", timeout=12)
    ...
    def get_admin_stats(self) -> dict:
        ...
        try:
            response = requests.get(f"{self.api_base}/api/v1/admin/stats", timeout=10)
            if response.status_code == 200:
                return response.json()
            return {}
        except Exception:
    ...
    def get_ai_insights(self) -> dict:
        ...
        try:
            response = requests.get(f"{self.api_base}/api/v1/admin/ai-insights", timeout=10)
            if response.status_code == 200:
                return response.json()
            return {}
        except Exception:
